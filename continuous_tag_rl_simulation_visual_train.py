...
# 속도 설정
# 단위: 1 step마다 이동하는 거리
CHASER_SPEED = 9.5 #술래속도를 9.5
RUNNER_SPEED = 7.0  # 도망자 속도도 7
...
SAVE_FILE = os.path.join(BASE_DIR, "continuous_tag_ai_learning_data_v2.pkl")
...
def move_position(position, action_index, speed):
    ...
    hit_wall = (new_x != raw_x) or (new_y != raw_y)
...
def random_start_positions():
    ...
    while True:
        ...
        if distance(chaser_pos, runner_pos) >= MIN_START_DISTANCE:
...
def make_agent_state(self_pos, opponent_pos):
    """
    AI가 보는 상태를 만든다.
    ...
    return angle_bin, dist_bin, x_wall, y_wall
...
def simulate_one_step(
    ...
    chaser_state = make_agent_state(chaser_pos, runner_pos)
    runner_state = make_agent_state(runner_pos, chaser_pos)
    ...
    # 술래가 도망자를 잡은 경우
    if new_distance <= CAPTURE_RADIUS:
        chaser_reward += 30.0
        runner_reward -= 30.0
        winner = "chaser"
    ...
    next_chaser_state = make_agent_state(new_chaser_pos, new_runner_pos)
    next_runner_state = make_agent_state(new_runner_pos, new_chaser_pos)
...
def play_one_episode(chaser_ai, runner_ai, epsilon, training=True, record=False):
    ...
    """
    chaser_pos, runner_pos = random_start_positions()
    trajectory = []
...
def demo_text(chaser_ai, runner_ai):
    ...
    for item in trajectory:
        ...
        if item["new_distance"] <= CAPTURE_RADIUS:
...
def train_episodes_visual(chaser_ai, runner_ai, episodes, total_episodes_done):
    ...
    def start_episode():
        ...
        state["chaser_pos"], state["runner_pos"] = random_start_positions()
